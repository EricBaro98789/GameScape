...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, path.join(__dirname, '../../Frontend/assets/images')); // 自定义保存路径
  },
...
const CollectedGame = sequelize.define('CollectedGame', {
  ...
  status: { type: DataTypes.STRING,  allowNull: true, defaultValue: 'WishList' },
  hoursPlayed: { type: DataTypes.INTEGER, allowNull: true, defaultValue: 0     },
  userRating:  { type: DataTypes.INTEGER, allowNull: true, defaultValue: 0     }
...
app.post('/register', upload.single('avatar'), async (req, res) => {
  try {
    ...
    if (req.file) {
      avatarUrl = "assets/images/" + req.file.filename;
    }
...
app.post('/login', async (req, res) => {
  try {
    ...
    const tokenPayload = { id: userInDb.id, email: userInDb.email, username: userInDb.username, avatar: userInDb.avatar };
    const token = jwt.sign(tokenPayload, JWT_SECRET, { expiresIn: '1h' });
    console.log(`User logged in successfully: ${userInDb.email} (ID: ${userInDb.id})`);
    res.json({
      message: 'Login successful!',
      token: token,
      user: { id: userInDb.id, username: userInDb.username, email: userInDb.email, avatar: userInDb.avatar }
    });
  } catch (error) {
...
app.post('/collection', async (req, res) => {
  ...
  const gameToSave = {
    ...
    rawgGameId:  parseInt(gameId, 10),
    gameTitle,
    gameImage,
    rating,
    notes:       typeof notes === 'string' ? notes : '',
    tags:        typeof tags === 'string' ? tags : '',
    status:      typeof status === 'string' ? status : 'WishList',
    hoursPlayed: parseInt(hoursPlayed) || 0,
    userRating:  parseInt(userRating) || 0
  };

  console.log('POST /collection: Saving to DB:', gameToSave);

  const [gameEntry, created] = await CollectedGame.findOrCreate({
    where: { userId: userIdFromToken, rawgGameId: gameToSave.rawgGameId },
    defaults: gameToSave
  });

  if (!created) {
    await gameEntry.update(gameToSave);
    console.log('✅ Updated existing entry:', gameEntry.id);
  } else {
    console.log('✅ Created new entry:', gameEntry.id);
  }

  const updatedCollection = await CollectedGame.findAll({ where: { userId: userIdFromToken } });
  res.status(201).json({
    message: 'Game saved (created or updated)',
